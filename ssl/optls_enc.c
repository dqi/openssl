...
#define OPTLS_MAX_LABEL_LEN     246
 ...
 */
int optls_hkdf_expand(SSL *s, const EVP_MD *md, const unsigned char *secret,
                             const unsigned char *label, size_t labellen,
                             const unsigned char *data, size_t datalen,
                             unsigned char *out, size_t outlen)
{
    const unsigned char label_prefix[] = "optls ";
    ...
    unsigned char hkdflabel[sizeof(uint16_t) + sizeof(uint8_t) +
                            + sizeof(label_prefix) + OPTLS_MAX_LABEL_LEN
                            + EVP_MAX_MD_SIZE];
    WPACKET pkt;
    ...
    if (!WPACKET_init_static_len(&pkt, hkdflabel, sizeof(hkdflabel), 0)
        ...
        WPACKET_cleanup(&pkt);
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_OPTLS_HKDF_EXPAND,
 ...
 */
int optls_derive_key(SSL *s, const EVP_MD *md, const unsigned char *secret,
                     unsigned char *key, size_t keylen)
{
    static const unsigned char keylabel[] = "key";

    return optls_hkdf_expand(s, md, secret, keylabel, sizeof(keylabel) - 1,
                             NULL, 0, key, keylen);
}
 ...
 * success  0 on failure.
 */
int optls_derive_iv(SSL *s, const EVP_MD *md, const unsigned char *secret,
                    unsigned char *iv, size_t ivlen)
{
    static const unsigned char ivlabel[] = "iv";

    return optls_hkdf_expand(s, md, secret, ivlabel, sizeof(ivlabel) - 1,
                             NULL, 0, iv, ivlen);
}

int optls_derive_finishedkey(SSL *s, const EVP_MD *md,
                             const unsigned char *secret,
                             unsigned char *fin, size_t finlen)
{
    static const unsigned char finishedlabel[] = "finished";

    return optls_hkdf_expand(s, md, secret, finishedlabel,
                          ...
                          unsigned char *outsecret)
{
    ...
    if (prevsecret == NULL) {
        ...
        if (!optls_hkdf_expand(s, md, prevsecret,
                                ...
                                size_t insecretlen)
{
    ...
    return optls_generate_secret(s, ssl_handshake_md(s), s->early_secret,
 ...
 * 0 on error.
 */
size_t optls_final_finish_mac(SSL *s, const char *str, size_t slen,
                             unsigned char *out)
{
    const EVP_MD *md = ssl_handshake_md(s);
    unsigned char hash[EVP_MAX_MD_SIZE];
    ...
    if (key == NULL
        ...
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_OPTLS_FINAL_FINISH_MAC,
                 ERR_R_INTERNAL_ERROR);
        goto err;
    }
 ...
 */
int optls_setup_key_block(SSL *s)
{
    const EVP_CIPHER *c;
    const EVP_MD *hash;
    ...
    if (!ssl_cipher_get_evp
        ...
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_OPTLS_SETUP_KEY_BLOCK,
                                    ...
                                    unsigned char *iv, EVP_CIPHER_CTX *ciph_ctx)
{
    ...
    if (!optls_hkdf_expand(s, md, insecret, label, labellen, hash, hashlen,
                           secret, hashlen)) {
        /* SSLfatal() already called */
        goto err;
    }
    ...
    if (!optls_derive_key(s, md, secret, key, keylen)
            || !optls_derive_iv(s, md, secret, iv, ivlen)) {
        /* SSLfatal() already called */
        goto err;
    }
...
int optls_change_cipher_state(SSL *s, int which)
{
        ...
        if (s->enc_read_ctx != NULL) {
            ...
            if (s->enc_read_ctx == NULL) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                         SSL_F_OPTLS_CHANGE_CIPHER_STATE, ERR_R_MALLOC_FAILURE);
                goto err;
            }
        }
    ...
    if (((which & SSL3_CC_CLIENT) && (which & SSL3_CC_WRITE))
        ...
        if (which & SSL3_CC_EARLY) {
            ...
            if (handlen <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                         SSL_F_OPTLS_CHANGE_CIPHER_STATE,
            ...
            if (sslcipher == NULL) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                         SSL_F_OPTLS_CHANGE_CIPHER_STATE, SSL_R_BAD_PSK);
                goto err;
            }
            ...
            if (md == NULL || !EVP_DigestInit_ex(mdctx, md, NULL)
                ...
                SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                         SSL_F_OPTLS_CHANGE_CIPHER_STATE, ERR_R_INTERNAL_ERROR);
    ...
    if (label == client_application_traffic) {
         ...
         */
        if (!optls_hkdf_expand(s, ssl_handshake_md(s), insecret,
    ...
    if (finsecret != NULL
            && !optls_derive_finishedkey(s, ssl_handshake_md(s), secret,
...
int optls_update_key(SSL *s, int sending)
{
...
int optls_export_keying_material(SSL *s, unsigned char *out, size_t olen,
                                 ...
                                 size_t contextlen, int use_context)
{
    ...
    if (EVP_DigestInit_ex(ctx, md, NULL) <= 0
            ...
            || !optls_hkdf_expand(s, md, s->exporter_master_secret,
                                  (const unsigned char *)label, llen,
                                  data, datalen, exportsecret, hashsize)
            || !optls_hkdf_expand(s, md, exportsecret, exporterlabel,
